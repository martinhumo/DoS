        ...
        *nb: atoms per molecule []
        ...
        for mol in np.arange(1,nm+1,1):
            for step in range(self.n_steps):
                ...
                L = np.sum([np.cross(data_rposi[i,:],data_vicm[i,:]*m[i]) for i in range(nb) ],axis=0) #angular momentum in xyz
                ...
                #Principal intertia  moments and axis
                eival, eivec = np.linalg.eig(J) 
                idx = eival.argsort()[::-1] #highest to lowest
                eival = eival[idx]
                eival[-1] = 0. #value next to 0, replace to didn't get nan when sqrt
                eivec = eivec[:,idx]
                Imom[step,mol-1,:] = eival
                
                J_ab = np.diag(eival[0:2]) #[[a,0],[0,Ib]]
                inv_J_ab = np.linalg.inv(J_ab)
                L_ab = np.array([np.dot(L,eivec[:,0]), np.dot(L,eivec[:,1])])
                
                #angular velocity in abc (molecule axis)
                CMws__abc = np.append( np.inner(inv_J_ab, L_ab), [0]) # [0] correspond to w around c axis(principal)
                
                #lines to avoid eigvect +1 o -1 directions
                
                if step == 0:
                    CMws[step,mol-1,:] = CMws__abc*np.sqrt(eival[:]) #See eq. 3.3 Master thesis 2PT bernhartdt
                    eivecprev = eivec
                else:
                    for i in range(3):
                        ...
                        CMws[step,mol-1,i] = CMws__abc[i]*np.sqrt(eival[i]) #srqt(I_jl)*w_jl
                    eivecprev = eivec
        ...
        *nb: atoms per molecule []
        ...
        for mol in np.arange(1,nm+1,1):
            for step in range(self.n_steps):
                ...
                CMws_xyz = np.dot(np.linalg.inv(J),L) #angular velocity in xyz
                CMws[step,mol-1,:] = CMws_xyz
